...
model = train(model, train_dataloader, val_dataloader, criterion, optimizer, DATASET, scheduler, num_epochs=num_epochs, save=True, device=device, 
              backbone=f'TL_{BACKBONE}_{backbone_mode}_3class_{LABEL}', train_sampler=train_sampler, is_main_process=is_main_process)

# %% [markdown]
# ### Test

#%%
path = os.path.join(DATASET, f'output/models/TL_{BACKBONE}_{backbone_mode}_3class_{LABEL}_best.pth')
...
if is_main_process:
    test(model, test_dataloader, saliency=True, device=device, save_prob=True,prob_name=f'TL_{BACKBONE}_{backbone_mode}_3class')
