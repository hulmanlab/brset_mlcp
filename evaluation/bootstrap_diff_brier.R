...
pairs <- c(
  combn(eval_files, 2, simplify = FALSE),
  combn(full_files, 2, simplify = FALSE)
)
...
for (pair in pairs) {
  ...
  mode <- get_mode(name1)
  model <- paste0(get_model(name1), " vs ", get_model(name2))
...
write.csv(brier_diff_results, file.path(prob_root, "summary", "Brier_diff_results.csv"), row.names = FALSE)
gc()