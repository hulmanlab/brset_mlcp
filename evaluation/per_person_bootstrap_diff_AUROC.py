...
eval_pairs = [pair for pair in eval_pairs if 'dinov3' in pair[0]]
full_pairs = list(combinations(full_lists, 2))
full_pairs = [pair for pair in full_pairs if 'dinov3' in pair[0]]
#%%
...
#%%
df_result.to_csv(os.path.join(prob_root, 'summary', 'AUROC_diff_results_per_person.csv'), index=False)
gc.collect()